...
```
This means the following: For the price of `r output_psm$data_vanwestendorp[260, "price"]`, ...

* `r round(output_psm$data_vanwestendorp[260, "ecdf_toocheap"], digits = 2) * 100`% of respondents think that this price is not too cheap (*not* too cheap because the function for the "too cheap" answers is reversed in every Price Sensitivity Meter analysis as explained in the original article)
* `r round(output_psm$data_vanwestendorp[260, "ecdf_cheap"], digits = 2) * 100`% of respondents think that this price is cheap
* `r round(output_psm$data_vanwestendorp[260, "ecdf_expensive"], digits = 2) * 100`% of respondents think that this price is expensive
* `r round(output_psm$data_vanwestendorp[260, "ecdf_tooexpensive"], digits = 2) * 100`% of respondents think that this price (or more) is too expensive
* `r round(output_psm$data_vanwestendorp[260, "ecdf_not_cheap"], digits = 2) * 100`% of respondents think that this price is *not* cheap - this is the inverse of `edfc_cheap` (so both values always add up to 100%)
* `r round(output_psm$data_vanwestendorp[260, "ecdf_not_expensive"], digits = 2) * 100`% of respondents think that this price is *not* expensive - this is the inverse of `edfc_expensive` (so both values always add up to 100%)
